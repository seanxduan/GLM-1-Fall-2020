...
gather() #grabs multiple colum labels and stacks them into a key column w/ values in a separate value
spread() #opposite of gather, takes a category 'key' column and makes each level in to a column, using 'value' to gather by
separate() 
unite()
...
weather3 <- spread(data = without_X, key = measure, value = value)
# This line of code is saying that we want to make a new data set called
...
test <- as.numeric(weather5$PrecipitationIn)
test
