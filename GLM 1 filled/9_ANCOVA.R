...
m2 <- lm(score ~  curriculum * teacher + pretest, data = ex10.1)  # cov at end 
summary(m2)
anova(m2)

anova(m2, m1)  # !Can get SS values from anovas ablve
...
dose <-c(rep(1, times = 9),rep(2, times = 8), rep(3, times = 13))        # unbalanced
...
viagra %>%                                                                    # tall data only
  gather(key = libido_type, value = libido, libido, p_libido, na.rm = T) %>%
...
anova(ssm3)
anova(ssm4)                          # If balanced, order of pred doesnt matter
...
Anova(ssm2, type = 2) # `type = 2` is the default             # All adjusted so that...(?)
...
Anova(lm(libido ~ dose + p_libido + dose:p_libido, data = viagra, 
         contrasts = list(dose = contr.Helmert(n = 3))), type = 3)
Anova(lm(libido ~ p_libido + dose + dose:p_libido, data = viagra, 
         contrasts = list(dose = contr.Helmert(n = 3))), type = 3)
