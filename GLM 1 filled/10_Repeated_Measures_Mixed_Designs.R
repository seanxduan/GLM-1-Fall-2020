...
##########################
# Fixing the Data Format #                                    # Need tall data for repeated (ie: one wn-subj var, one column)
##########################
...
bush_tall <- bush_tall[order(bush_tall$participant),]                         ## Changes struct of actual data
...
# This new error term accounts for the within-subject error in `participant` for `animal`.# Bc meas for anim repeated
...
#########################
# Our model using lme() #                                                               # lme() allows the modeling of error
#########################

m2_bush <- lme(retch ~ animal, 
                  random = ~ 1|participant/animal,      # --> for outcome, same values of "participant" wn diff levels of "animal" are dep
...
speed_tall <- speeddate %>% 
  ...
  mutate(personality = gl(3, 60, labels = c("Charismatic", "Average", "Dullard"))) %>% 
  mutate(looks = gl(3, 20, 180, labels = c("Attractive", "Average", "Ugly")))
...
baseline_date <- lme(date_rating ~ 1, 
                        random = ~ 1|participant/personality/looks,     # Biggest down to smallest
